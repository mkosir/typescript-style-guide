...
Function conventions should be followed as much as possible (some of the derives from functional programming basic concepts):
...
- use `undefined` assignment when trying to exclude fields. E.g. in form fields, request payload, querying database ([Prisma differentiation](https://www.prisma.io/docs/concepts/components/prisma-client/null-and-undefined)) etc.
...
<p>
  ...
  <a href="https://typescript-eslint.io/rules/consistent-type-definitions/#type" target="_blank">
    (eslint rule)
  </a>
  .<Chip>Consider using interfaces if developing package that can be further extended, team is more comfortable working with interfaces etc. Disable lint rule in case of exceptions as type unions (type Status = 'loading' | 'error') etc.</Chip>
</p>
...
<p>
  As rule of a thumb, use type annotation when it helps narrow a type.
  <Chip>
    Just because you don't need to add types, doesn't mean you shouldn't. In some cases explicit type annotations can
