...
When dealing with ambiguous data type use `unknown` since it does not allow dereferencing all properties.

```ts
// ❌ Avoid any
const foo: any = 'five';
let bar: number = bar; // no type error

// ✅ Use unknown and narrow the type before dereferencing it
const foo: unknown = 5;
let bar: number = foo; // type error - Type 'unknown' is not assignable to type 'number'
```
