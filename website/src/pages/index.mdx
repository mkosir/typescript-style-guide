...
When dealing with ambiguous data type use `unknown` since it does not allow dereferencing all properties (anything can be assigned to `unknown`, but `unknown` isn’t assignable to anything).
