...
Discriminated unions are a powerful concept to model complex data structures and improve type safety, leading to clearer, more maintainable, and less error-prone code.  
...
- **Object constants**
  ...
  If object type exist use `satisfies` operator, to verify you object constant matches its type.
...
Creating more complex types often include generics, which can make them hard to read and understand, that's why we try to put best effort naming them.  
Naming generics using popular convention with one character `T`, `K` etc. is not allowed, the more variables we introduce, the easier it is to mistake them.
