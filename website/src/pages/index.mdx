...
- array becomes `readonly` tuple
  ...
  const getShapeArea = (shape: Shape) => {
    switch (shape.kind) { // Error - Switch is not exhaustive. Cases not matched: "circle"
      case 'square':
        return shape.size * shape.size;
      case 'rectangle':
        return shape.width * shape.size; // Error - Property 'size' does not exist on type 'kind'
    }
  };
  ```
