...
Template literal types are useful in various scenarios, such as:
...
As rule of a thumb try to use literal type most frequently, const assertion arrrays when there is a need to loop through the values and need for const assetion objects should happen rarely:

- Majority of times there is no need to create run time object that add up to the bundle size, we only need literal type at compile time.

  ```ts
  // ❌ Avoid using enums adding up to the bundle size
  enum UserRole {
    GUEST = 'guest',
    MODERATOR = 'moderator',
...
- Use const assertion arrays, when need to loop through the values.
...
- Use const assertion objects when there is a strong use case for it.
